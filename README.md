...
## maintenance
### influxdb
+ if influxdb is purged, we have to manually create a new 'basecamp' database (here with a retention policy of 6 months):
`docker run --rm --link=influxdb -it influxdb:alpine influx -host influxdb` then `CREATE DATABASE basecamp WITH DURATION 24w`
